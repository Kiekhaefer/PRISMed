...
class PrisMedKafkaResourceIT {
    ...
    void setup() {
        ...
        PrisMedKafkaResource kafkaResource = new PrisMedKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/pris-med-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/pris-med-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
